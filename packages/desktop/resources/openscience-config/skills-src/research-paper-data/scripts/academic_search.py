...
# ///
"""No-MCP fallback search adapted from Yuan1z0825/nature-skills.

OpenScience bundles this Apache-2.0 script inside research-paper-data so paper
search has a local, public-API fallback without any OpenScience backend. The
OpenScience copy adds a Crossref fallback for plain topic searches when
OpenAlex is temporarily unavailable.

Original role in nature-skills: no-MCP fallback search for
nature-academic-search (OpenAlex, stdlib only).
...
def crossref_search(query: str, limit: int = 10, year_from: int | None = None) -> list[dict]:
    ...
    for item in ((data.get("message") or {}).get("items") or [])[:limit]:
        ...
        results.append({
            ...
            "doi": item.get("DOI", ""),
            "authors": _crossref_authors(item)[:5],
            "year": year,
...
def main():
    ...
    MAILTO = (args.mailto or os.environ.get("OPENALEX_MAILTO")
              ...
              or os.environ.get("OPENSCIENCE_CONTACT_EMAIL") or MAILTO)
    ...
    try:
        ...
        if args.query and not author_id and not args.author and not args.orcid:
            print(f"WARNING: OpenAlex returned HTTP {e.code}: {e.reason}{hint}; trying Crossref fallback",
                  file=sys.stderr)
            try:
                results = crossref_search(args.query, args.limit, args.year_from)
            except Exception as fallback_error:
                print(f"ERROR: Crossref fallback failed: {fallback_error}", file=sys.stderr)
                return 1
        else:
        ...
        if args.query and not author_id and not args.author and not args.orcid:
            print(f"WARNING: OpenAlex network request failed: {e}; trying Crossref fallback", file=sys.stderr)
            try:
                results = crossref_search(args.query, args.limit, args.year_from)
            except Exception as fallback_error:
                print(f"ERROR: Crossref fallback failed: {fallback_error}", file=sys.stderr)
        ...
        if args.query and not author_id and not args.author and not args.orcid:
            print("WARNING: OpenAlex returned a non-JSON response; trying Crossref fallback", file=sys.stderr)
            try:
                results = crossref_search(args.query, args.limit, args.year_from)
            except Exception as fallback_error:
                print(f"ERROR: Crossref fallback failed: {fallback_error}", file=sys.stderr)
    ...
    if args.compact:
        for r in results:
            ...
            print(f"[{r['year']}] {r['title']} | {authors_str} | {r['journal']} | DOI:{r['doi']} | Cited:{r['cited_by_count']}{source}")
    else:
